...
export function WithdrawalRow({ withdrawalId }: { withdrawalId: string }) {
  const router = useRouter();
  const [note, setNote] = useState("");
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");

  async function act(status: "completed" | "rejected") {
    setLoading(true);
    setError("");
    try {
      const res = await fetch("/api/admin/withdrawal", {
        ...
        body: JSON.stringify({ withdrawalId, status, adminNote: note }),
      });
